# SFRD-TS-02-3.5_XX版本XXX模块概要设计说明书
...
| 2.4 | 可靠性要求 | 本业务模块的可靠性要求（也即可靠性能力目标） | > 定义本模块的可靠性能力目标（目标要可验证可衡量） |
| 2.5 | 可测试性要求 | 可测试性目标或需求 |  |
| 2.6 | 可调试性要求 | 可调试性目标或需求 |  |
| 2.7 | 可运维性要求 | 可运维性目标或需求 |  |
| 2.8 | 可扩展性要求 | 可扩展性目标或需求 |  |
| 2.9 | 可复用性要求 | 可复用性目标或需求 |  |
| 2.10 | 兼容性 | 允许升级配置的版本以及允许对接的版本 |  |
| 2.11 | 其它 |  |  |

## 3. 对外接口

> 描述本模块需要提供哪些对非本模块的功能接口以及对应的支持自动化测试接口，对外接口需要遵守公司定义的API规范

### 3.1. API接口

> 如果未对外提供API接口，本章节可以忽略。Api接口：以api或rpc等形式提供给外部模块的接口。

### 3.2. 消息接口

> 如果未对外提供消息接口，本章节可以忽略。消息接口：以进程/实体间传递的消息为接口
...
> 对本模块在客户处的应用场景进行分析。客户应用本模块解决什么问题，客户在哪些场合、有哪些方式来应用本模块，使用过程中有哪些操作，会发生哪些事件，需要得到什么处理结果？比如：在京东购书，主场景是用户登录app，搜索图书，下单，付款，快递上门。过程中可能发生登录失败，付款不成功，等各种各样的事件。场景分析的目的是分析清楚模块运行过程中会发生什么事件，分析的结果用以评估各方案在实际运行过程中，能否正常应对这些事件，会不会发生问题。可以采用用例分析（use case）方法对场景进行分析，也可以用事件流分析方法对场景进行分析。
...
> 评估准则一般围绕性能、可靠性、可扩展性、易实现以及高可用等。
>
> 等级使用高、中以及低三个等级。

| 方案名称 | 评估准则1      | 评估准则2      | 评估准则3      | 评估准则4      | 评估准则5      |
| -------- | -------------- | -------------- | -------------- | -------------- | -------------- |
| 方案1    | 高，[具体描述] | 中，[具体描述] | 中，[具体描述] | 低，[具体描述] | 高，[具体描述] |
| 方案2    |                |                |                |                |                |
| 方案3    |                |                |                |                |                |
...
> 模板下载处：https://wiki.sangfor.com/pages/viewpage.action?pageId=81603752 中的《 模块概要设计-安全兜底机制合入checklist.xlsx》
...
> 威胁建模学习资源：https://wiki.sangfor.com/pages/viewpage.action?pageId=81602196

> 威胁建模分析报告模板：https://wiki.sangfor.com/pages/viewpage.action?pageId=81603752 中的《SFRD-SDL-TMSD-1.5_X产品X.X威胁建模分析 .xlsx》

##### 4.6.1.3. 安全设计

> 根据上一节威胁建模中识别出的风险，设计相关的安全机制，以确保系统的安全性。整体安全架构设计和各模块的设计都要满足https://wiki.sangfor.com/pages/viewpage.action?pageId=81603752 中的《安全需求识别checklistV1.2-20241125.xlsx》中的涉及项。

> 最佳安全设计可参考：https://wiki.sangfor.com/pages/viewpage.action?pageId=81602113
...
> 指导：Mend扫描指导文档见：https://wiki.sangfor.com/pages/viewpage.action?pageId=81602677

> 组件不符合安全要求且无法替换时，需备案：由安全经理对问题组件完成风险分析，并向安全研发部吴义和11582，抄送研发主管发出邮件申请和确认，将扫描结果、分析过程及研发主管的邮件答复结果上传至该处。

#### 4.6.2. 可靠性设计

> 基于可靠性工程的角度来进行的，基于故障容错的思路来开展的，**不**包含**代码功能实现缺陷、业务逻辑的不合理、业务条件的不满足**导致的特性失效。主要包含下面三个方面的内容，其中前两个内容主要依赖于系统内的公共机制来辅助实现或者依赖特性功能自身实现，第三个内容依赖FMEA分析来进行：
...
> FMEA相关指导和链接如下：
> FMEA分析指导：特性设计指导中的2.3
> FMEA分析模板：公司级FMEA分析模板
> 可靠性需求基线：公司级可靠性需求基线
> 可靠性设计准则：公司级可靠性设计准则
...
> 可测试性机制设计的目的是降低后续测试的难度，提高测试工作效率和质量，尽早发现问题。为了提升可测试性设计的质量，设计者需要遵守《软件可测试性技术规范》的要求，详情请参考：软件可测试性技术规范
